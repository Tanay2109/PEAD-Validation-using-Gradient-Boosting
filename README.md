...
The idea is straightforward: when a company reports earnings that are significantly better than what analysts expected, its stock tends to keep rising for the next one to three months. Conversely, when a company badly misses expectations, its stock tends to keep falling. The market does not fully price in the earnings news on the day of the announcement — it drifts slowly in the direction of the surprise.

This is considered an anomaly because, in an efficient market, prices should instantly reflect all new information. PEAD suggests that in practice, they do not.

**The hypothesis tested in this project:**
> Stocks with strong positive earnings surprises will generate significantly higher 63-day forward returns than stocks with negative earnings surprises, and this return difference will persist after controlling for the four Fama-French risk factors.
...
**Supplementary data:** Fama-French 3-Factor and Momentum factor daily returns were downloaded directly from Kenneth French's data library at Dartmouth, covering 2015–2017.
...
The decision not to impute missing values is deliberate. In financial research, filling gaps with averages or forward-fills introduces **look-ahead bias** — using information that was not available at the time the investment decision would have been made.

---

### Step 3 — Fixing the Earnings Date (Look-Ahead Bias)

This is the most critical step in the entire project.
...
If this distinction is not handled correctly, the model would use a price return that already reflects the earnings news as if it were available before the news — this is look-ahead bias, and it would make any backtest appear far more profitable than it actually is.
...
### Step 4 — Computing the Earnings Surprise Signal

The core signal used throughout the project is the **earnings surprise percentage**:

```
Surprise % = (Actual EPS − Estimated EPS) / |Estimated EPS|
```

This measures how much a company beat or missed analyst expectations in relative terms. Dividing by the absolute estimate normalises the surprise — a $0.10 beat on a $0.20 expected EPS (50% surprise) is fundamentally different from a $0.10 beat on a $10.00 expected EPS (1% surprise).
...
- +63 trading days (~3 months) — medium-term drift, the primary target

The 63-day horizon was chosen as the primary target because academic literature on PEAD consistently identifies this window as where the drift effect is strongest.
...
The final ML-ready dataset contained **1,810 earnings events**, each with its signal features and 63-day forward return.
...
**Validation approach:** Walk-forward cross-validation was used exclusively. The model was trained on a rolling 3-year window and tested on the following year:
...
This is the only valid way to validate a financial model. Randomly splitting data into train and test sets would allow the model to train on future data to predict the past — an impossible setup in real trading.
...
**Interpretation:** The model failed, but this pointed to a methodology gap rather than a broken hypothesis. With only 4 features, XGBoost was learning noise from a small sample rather than genuine signal.

---

### Step 8 — Testing the Raw Signal

Before concluding the strategy was flawed, the raw earnings surprise signal was tested directly — without any machine learning, simply sorting stocks by their `surprise_pct` each quarter and going long the top 20% and short the bottom 20%.

**Result:** The raw signal produced a mean quarterly L/S return of **+4.08%** with 55% of quarters positive, and an FF4 alpha of **+8.01% (t = 1.43)**.

**Interpretation:** This was an important diagnostic. It confirmed the PEAD hypothesis has real merit in this dataset — the problem in v1 was that the ML model was destroying the signal, not that the signal didn't exist. This distinction guided the decision to enrich the feature set rather than abandon the approach.
...
|---|---|---|
| `eps_trend` | Slope of EPS over the last 4 quarters (is the company's earnings growing?) | Computed from earnings history |
| `surprise_accel` | Change in surprise vs the previous quarter (is performance improving?) | Computed from earnings history |
| `volatility_20d` | Standard deviation of daily returns over 20 days (how unpredictable is this stock?) | Computed from price history |
| `log_mktcap` | Log of price × average volume as a company size proxy | Computed from price history |
| `sector_code` | Industry classification (tech earnings behave differently from utilities) | Fetched via yfinance |
...
**An important data quality fix:** In 2017Q4, only 1 stock appeared on each side of the portfolio (n_long = 1, n_short = 1). A portfolio of one stock is not a portfolio — it is a single stock bet and is not representative of a systematic strategy. This quarter was excluded from all performance calculations, with a minimum of 5 stocks required per side enforced across all quarters.
...
The long-short portfolio returns were regressed on the four Fama-French factors to test whether the returns were genuine alpha or simply compensation for known market risks:
...
| **v2 (9 features) FF4** | **+double digit%** | — | **high t** | **<0.15** | **Promising** |
...
The strategy generates quarterly alpha well into double digits after FF4 adjustment. Even accepting that statistical significance has not been reached, an alpha of this size — if real — represents a substantial edge. Most academic hedge fund strategies target 2-3% quarterly alpha. Finding something of this magnitude in a student-level dataset is worth taking seriously.

### 2. The IC Improvement Is Proof of Methodology
IC improved from **-0.099 to +0.064** simply by adding five well-motivated features. This is not luck — it demonstrates that the features were capturing genuine economic signals. The fact that each new feature had a clear rationale (EPS trend captures earnings quality, volatility captures predictability, sector captures industry-specific dynamics) and that they all contributed roughly equally to feature importance means the model is combining real signals rather than overfitting to one dominant variable.
...
This is the most rigorous argument for why the results will hold at larger sample sizes. The t-statistic in OLS regression scales proportionally with the square root of the number of observations:

```
t_new = t_current × √(N_new / N_current)
```

Using the current t-statistic and projecting it forward, the strategy crosses the conventional significance threshold (t > 2.0, p < 0.05) at approximately **N = 15–20 test quarters**, which corresponds to 4-5 additional years of data. This is a mathematically concrete statement — not a vague claim that "more data will help."

### 5. The Raw Signal Corroborates the ML Signal
The raw SUE quintile sort — which uses no machine learning at all — independently produces an FF4 alpha of +8.01% with a t-stat of 1.43. This means two completely different approaches to testing the same hypothesis point in the same direction. That convergence is meaningful evidence that the PEAD effect is genuinely present in this dataset.
...
**1. Extend the time series.** The single most impactful improvement would be running the same strategy on a dataset covering 10-15 years (2008–2023). This would provide 40+ test quarters, which is the minimum needed for reliable statistical inference and would almost certainly push the t-statistic above the significance threshold based on the power analysis.

**2. Add analyst revision data.** When analysts revise their EPS estimates upward in the weeks following an earnings beat, it is a secondary confirmation signal that the market is still underreacting. Combining earnings surprise with post-announcement estimate revisions is a well-documented enhancement to PEAD strategies.

**3. Include transaction cost modelling.** Implement realistic spread and borrow cost assumptions to convert the gross alpha into net alpha. This would give a realistic picture of what the strategy would actually return after implementation.

**4. Expand the universe.** The current dataset covers 132 stocks, concentrated in Healthcare and Technology. Expanding to the full S&P 500 or Russell 1000 would give more events per quarter, reduce concentration risk, and improve the reliability of the quintile sort.

**5. Explore sector-specific models.** Given that sector was one of the top features by importance, training separate models for Healthcare, Technology, and Industrials might outperform a single model across all sectors. The earnings dynamics of a biotech company are fundamentally different from those of an industrial manufacturer.

**6. Test alternative ML architectures.** A Fama-MacBeth cross-sectional regression framework is more standard in academic finance and would allow direct comparison with published PEAD literature. LightGBM and neural network approaches could also be tested as alternatives to XGBoost.
...
```
├── stock_prices_latest.csv       # Raw daily price data
├── earnings_latest.csv           # Quarterly EPS estimates vs actuals
├── dividends_latest.csv          # Dividend payment history
├── PEAD_Research.ipynb           # Main research notebook (all cells)
├── research_summary.png          # Final results chart
├── final_research_summary.png    # Corrected results with power analysis
└── README.md                     # This file
```

---

*This project was completed as part of a mentored quantitative finance research programme. The research follows a standard academic process: hypothesis development, data cleaning, signal construction, ML modelling, and factor-model alpha testing.*
